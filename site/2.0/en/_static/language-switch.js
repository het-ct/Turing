(function () {
  ...
  function init() {
    ...
    var version = match[1], language = match[2], remainder = match[3] || "/";
    var page = remainder.replace(/^\//, "") || "index.html"; if (page.charAt(page.length - 1) === "/") page += "index.html";
    var labels = { zh: "简体中文", en: "English" };
    function makeUrl(targetLanguage, targetPage) { var base = window.location.pathname.replace("/" + version + "/" + language + remainder, "/" + version + "/" + targetLanguage + "/"); return window.location.origin + base + (targetPage === "index.html" ? "" : targetPage) + window.location.search + window.location.hash; }
    ...
    function createSwitch(cls) { var wrapper = document.createElement("label"), select = document.createElement("select"); wrapper.className = "sdk-language-switch " + cls; wrapper.setAttribute("aria-label", language === "zh" ? "切换语言" : "Switch language"); select.title = language === "zh" ? "切换语言" : "Switch language"; ["zh", "en"].forEach(function (item) { var option = document.createElement("option"); option.value = item; option.textContent = labels[item]; option.selected = item === language; select.appendChild(option); }); select.addEventListener("change", function () { go(makeUrl(select.value, page), makeUrl(select.value, "index.html")); }); wrapper.appendChild(select); return wrapper; }
    var sidebar = document.querySelector(".wy-side-nav-search"); if (sidebar) { sidebar.querySelectorAll(".sdk-language-switch").forEach(function (node) { node.remove(); }); if (!sidebar.querySelector(".sdk-doc-title")) { var title = document.createElement("div"); title.className = "sdk-doc-title"; title.textContent = language === "zh" ? "Het-X1 SDK 开发手册" : "Het-X1 SDK Development Manual"; sidebar.insertBefore(title, sidebar.firstChild); } }
